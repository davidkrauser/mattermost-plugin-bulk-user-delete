...
const USAGE = "[mode] [target active]"
...
func (p *Plugin) ExecuteCommand(_ *plugin.Context, args *model.CommandArgs) (*model.CommandResponse, *model.AppError) {
	...
	if dryRun {
		return &model.CommandResponse{
			...
			Attachments: []*model.SlackAttachment{{
				Pretext: fmt.Sprintf("Bulk user deletion would remove the following %d users:", len(usersToDelete)),
				Text:    userListString,
			}},
		}, nil
	}
	...
	return &model.CommandResponse{
		ResponseType: model.CommandResponseTypeInChannel,
		Attachments: []*model.SlackAttachment{{
			Pretext: fmt.Sprintf("Starting bulk user deletion job. When complete, the result will be posted in this channel. The following %d users are being deleted:", len(usersToDelete)),
			Text:    userListString,
		}},
	}, nil
}
